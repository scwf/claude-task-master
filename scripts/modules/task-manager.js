				...
				try {
					...
					if (modelType === 'perplexity') {
						...
						const deepseekModel =
							...
							session?.env?.DEEPSEEK_MODEL ||
							'deepseek-chat';
						
						log('info', `使用DeepSeek模型: ${deepseekModel}`);
...
Note on dependencies: Subtasks can depend on other subtasks with lower IDs. Use an empty array if there are no dependencies.`;
				...
				while (modelAttempts < maxModelAttempts && !responseText) {
					modelAttempts++;
					try {
						// 获取可用的AI模型
						...
						report(`使用${modelType}模型生成子任务（第${modelAttempts}次尝试）`);
						
						if (modelType === 'perplexity' || modelType === 'deepseek') {
							// 使用OpenAI兼容API的客户端（Perplexity或DeepSeek）
							const model = modelType === 'perplexity' 
								? (process.env.PERPLEXITY_MODEL || session?.env?.PERPLEXITY_MODEL || 'sonar-pro')
								: (process.env.DEEPSEEK_MODEL || session?.env?.DEEPSEEK_MODEL || 'deepseek-chat');
							
							report(`使用${modelType}模型: ${model}`);
						...
						report(`第${modelAttempts}次尝试失败: ${error.message}`, 'warn');
						
						// 检查是否是Claude过载错误
						const isOverload = error.message?.toLowerCase().includes('overload') ||
						...
						if (isOverload) {
							claudeOverloaded = true;
							report('Claude过载，将尝试备选模型', 'warn');
							// 继续循环尝试下一个模型
						} else if (modelAttempts >= maxModelAttempts) {
							// 最后一次尝试失败，抛出错误
							throw error;
						}
					}
				...
				// 如果没有成功获取响应，抛出错误
				if (!responseText) {
					throw new Error('所有可用模型均未能生成子任务');
				}
			...
			try {
				...
				while (modelAttempts < maxModelAttempts && !aiGeneratedTaskData) {
					...
					try {
						...
						if (modelType === 'perplexity' || modelType === 'deepseek') {
							// 使用OpenAI兼容的API (Perplexity或DeepSeek)
							const modelName = modelType === 'perplexity' 
								? (process.env.PERPLEXITY_MODEL || session?.env?.PERPLEXITY_MODEL || 'sonar-pro')
								: (process.env.DEEPSEEK_MODEL || session?.env?.DEEPSEEK_MODEL || 'deepseek-chat');
							
							log('info', `使用${modelType}模型: ${modelName}`);
...
/**
 * Get the color code for a priority value
...
/**
 * Update a task by ID with new information from AI
 * @param {string} tasksPath - Path to the tasks.json file
 * @param {number|string} taskId - Task ID or subtask ID to update
 * @param {string} prompt - Prompt with the update context
 * @param {boolean} useResearch - Whether to use Perplexity for research
 * @param {Object} options - Options object containing reportProgress, mcpLog and session
 * @param {string} outputFormat - Output format (text or json)
 * @returns {Object|null} Updated task or null if task not found or completed
 */
async function updateTaskById(
    ...
    outputFormat = mcpLog ? 'json' : outputFormat;
    ...
    try {
        report(`Updating task ${taskId} with prompt: "${prompt}"`, 'info');
        ...
        const taskToUpdate = findTaskById(data.tasks, taskId);
        if (!taskToUpdate) {
            throw new Error(`Task with ID ${taskId} not found`);
        }
            ...
            if (outputFormat === 'text') {
                console.log(
                    boxen(
                        ...
                        '\n\n' +
                        chalk.white('Completed tasks are locked to maintain consistency.'),
                        { padding: 1, borderColor: 'yellow', borderStyle: 'round' }
                    )
                );
            }
        ...
        const systemPrompt = `You are an AI assistant helping to update software development tasks. 
Given a task and new information or context, provide an updated version of the task 
that incorporates this new information while preserving existing content structure.`;

        // Initialize variables for fallback handling
        let updatedTask = null;
        let claudeOverloaded = false;
        let modelAttempts = 0;
        const maxModelAttempts = 2;

        // Loading indicator (only for text output)
        let loadingIndicator = null;
        if (outputFormat === 'text' && !isSilentMode()) {
            loadingIndicator = startLoadingIndicator('Generating updated task with AI...');
        }
        ...
        try {
            ...
            while (modelAttempts < maxModelAttempts && !updatedTask) {
                ...
                try {
                    ...
                    report(`Attempt ${modelAttempts}/${maxModelAttempts}: Generating updated task using ${modelType}`, 'info');

                    // Build the AI prompt
                    const taskString = JSON.stringify(taskToUpdate, null, 2);

                    // Prepare user message
                    const userMessageContent = `Here is the current task:
${taskString}

Update this task based on the following new information:
${prompt}

Return the complete updated task as a single JSON object. Maintain the same field structure.
Preserve completed subtasks and their attributes.`;

                    // Call the AI
                    let responseText = await sendChatWithContext(
                        client,
                        {
                            model: session?.env?.ANTHROPIC_MODEL || CONFIG.model,
                            max_tokens: session?.env?.MAX_TOKENS || CONFIG.maxTokens,
                    ...
                    if (jsonStart === -1 || jsonEnd === -1) {
                        throw new Error(`Could not find valid JSON in ${modelType}'s response`);
                    }
                    ...
                    if (!updatedTask || typeof updatedTask !== 'object') {
                        throw new Error('Invalid task object in AI response');
                    }
                } catch (modelError) {
                    report(`Attempt ${modelAttempts} failed using ${modelType}: ${modelError.message}`, 'warn');
                    
                    // Handle overloaded model
                    if (modelError.message?.toLowerCase().includes('overload') && !isLastAttempt) {
                        if (modelType === 'claude') {
                            claudeOverloaded = true;
                            report('Will try with alternative model next', 'info');
                        }
                        continue;
                    } else if (isLastAttempt) {
                        report(`Final attempt failed. No fallback possible.`, 'error');
                        throw modelError;
                    } else {
                        throw modelError;
                    }
            ...
            // Validate and clean up the updated task
            if (!updatedTask) {
                throw new Error('Failed to generate updated task after all attempts');
            }

            // Ensure critical fields exist
            if (!updatedTask.title || !updatedTask.description) {
                throw new Error('Updated task is missing required fields');
            }

            // Ensure ID is preserved
            updatedTask.id = taskToUpdate.id;

            // Ensure status is preserved unless explicitly mentioned in prompt
            ...
            if (taskToUpdate.subtasks && taskToUpdate.subtasks.length > 0) {
                if (!updatedTask.subtasks) {
                    ...
                    for (const completedSubtask of completedSubtasks) {
                        ...
                        if (!updatedSubtask) {
                            updatedTask.subtasks.push(completedSubtask);
                        } else if (updatedSubtask.status !== completedSubtask.status) {
                            updatedSubtask.status = completedSubtask.status;
                        }
                    }
                }
            ...
            // Only show success UI for text output (CLI)
            if (outputFormat === 'text') {
                console.log(
                    boxen(
                        chalk.green(`Successfully updated task #${taskId}`) +
            ...
            // Clean up loading indicator if it was created
            if (loadingIndicator) {
                stopLoadingIndicator(loadingIndicator);
            }
        }
